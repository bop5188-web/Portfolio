...
function buildAdditionalCaseStudySections(page) {
  if (page === "graph") {
    return [
      createCaseStudySection("Research", [
        "I tested this with classmates who were learning BFS/DFS and noticed they understood results faster when they could edit the graph directly.",
        "The main feedback was to make outputs more explicit so users could quickly verify if the algorithm did what they expected."
      ]),
      createCaseStudySection("Key Insights", [
        "Hands-on interaction teaches algorithm behavior better than static text alone.",
        "Users needed side-by-side structure + output (controls, result box, matrix) to feel confident in what they were seeing."
      ]),
      createCaseStudySection("Testing + Iteration", [
        "I refined button labels and output messages after peer testing so each action clearly maps to one result.",
        "I added matrix and status feedback to reduce confusion when changing edges and rerunning algorithms."
      ]),
      createCaseStudySection("My Role", ["Java graph implementation", "UI design for algorithm explanation", "Front-end coding in JavaScript"])
    ];
  }
  ...
  if (page === "art-gallery") {
    return [
      createCaseStudySection("Research", [
        "I tested typical class prompts (artists, exhibitions, styles) and tuned responses for clarity and consistency.",
        "I compared short and long answer styles and found short responses were easier to scan and more useful."
      ]),
      createCaseStudySection("Key Insights", [
        "Users preferred concise answers with a clear next step.",
        "Grounded, curated content improved trust compared to broad open-ended responses."
      ]),
      createCaseStudySection("Testing + Iteration", [
        "I tightened prompt behavior and fallback responses after testing question variations.",
        "I rebuilt the portfolio version to run directly on-site so recruiters can test it immediately without setup."
      ]),
      createCaseStudySection("My Role", ["Prompt + conversation flow design", "Chat UI logic", "Integration-oriented front-end implementation"])
    ];
  }
  ...
  if (page === "sql") {
    return [
      createCaseStudySection("Research", [
        "I mapped expected business questions first, then built schema relationships around those reporting needs.",
        "I used output tables from the assignment to validate whether each query structure was correct."
      ]),
      createCaseStudySection("Key Insights", [
        "Well-planned table relationships make query debugging much faster.",
        "Clear aliasing and grouping strategy prevents most aggregation mistakes in larger joins."
      ]),
      createCaseStudySection("Testing + Iteration", [
        "I iterated query structure when early versions returned incorrect counts or ordering.",
        "I refined subquery and grouping logic until results matched expected department and employee outputs."
      ]),
      createCaseStudySection("My Role", ["Schema design in MySQL Workbench", "SQL query writing", "Validation + debugging"])
    ];
  }
...
function renderProjectDetail(page) {
  ...
  buildAdditionalCaseStudySections(project.page).forEach((section) => left.appendChild(section));
...
function renderNeurodivergentProject(project) {
  ...
  function applyVisualSettings() {
    ...
    if (paceSupport.value === "extended") body.classList.add("nd-pace-extended");
    if (paceSupport.value === "pause") body.classList.add("nd-pace-pause");

    applyTextTransformSettings();
  }

  function applyTextTransformSettings() {
    // Explicit text size updates so changes are clearly visible on the live site.
    const textTargets = document.querySelectorAll(
      ".project-desc, .output, .mini-title, .section-title, p, li, label, .nav-link, .btn, th, td, input, select"
    );
    textTargets.forEach((el) => {
      if (!el.dataset.baseFontSize) {
        const computed = window.getComputedStyle(el).fontSize;
        el.dataset.baseFontSize = computed;
      }
    ...
    if (readingPreference.value === "large") {
      textTargets.forEach((el) => {
        ...
        el.style.fontSize = `${Math.max(base + 4, 18)}px`;
    ...
    if (readingPreference.value === "dyslexia") {
      textTargets.forEach((el) => {
        const base = parseFloat(el.dataset.baseFontSize || "16");
        el.style.fontSize = `${Math.max(base + 2, 17)}px`;
    ...
    // Guided mode: convert descriptive text blocks into numbered, step-by-step format.
    const guidedTargets = document.querySelectorAll(".project-desc");
    guidedTargets.forEach((el) => {
      if (!el.dataset.originalGuideText) {
        el.dataset.originalGuideText = el.textContent || "";
      }
      if (navStyle.value !== "guided") {
        el.textContent = el.dataset.originalGuideText;
        return;
      }
      const original = (el.dataset.originalGuideText || "").trim();
      if (!original) return;
      const parts = original
        ...
        .filter(Boolean);
      if (parts.length < 2) {
        el.textContent = `Step 1: ${original}`;
      } else {
        el.textContent = parts.map((s, i) => `Step ${i + 1}: ${s}`).join(" ");
      }
    });
  }
