...
function renderNeurodivergentProject(project) {
  ...
  const actions = createEl("div", { className: "project-actions" }, []);
  const recommendBtn = createEl("button", { className: "btn btn-primary" }, ["Generate recommendations"]);
  ...
  function renderRecommendations() {
    ...
    output.appendChild(
      createEl("div", { className: "project-desc" }, [
        "Why this matters: these settings lower cognitive load, improve reading comfort, and make the interface more predictable."
      ])
    );
  }
  ...
  resetBtn.addEventListener("click", () => {
    ...
    output.textContent = "Choose preferences, then click Generate recommendations.";
  });
  ...
  if (url) {
    ...
    const btn = createEl("button", { className: "btn btn-ghost" }, ["Open original prototype link"]);
