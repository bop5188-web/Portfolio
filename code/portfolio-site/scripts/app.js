...
const PROJECTS = [
  ...
  },
  {
    ...
    short:
      "Accessibility-centered web prototype focused on reducing cognitive overload and improving usability for neurodivergent users.",
    tags: ["UX Research", "Accessibility", "Figma"],
    page: "neurodivergent",
    embed: {
      url: "https://briepo.myportfolio.com/wireframe-prototypes"
    }
  },
  {
    id: "storefront-sql",
...
function renderProjectDetail(page) {
  ...
  if (project.page === "neurodivergent") {
    story.appendChild(
      createEl(
        ...
        { className: "output" },
        [
          "I designed this project to explore how websites can better support neurodivergent users. I focused on reducing cognitive overload by simplifying layout, improving readability, and making navigation feel calmer and more predictable. I used a prototype-first approach so I could test ideas early and adjust based on feedback before building too much. This project strengthened my accessibility thinking and how I make design decisions based on user comfort, not just visuals. It also showed me how small hierarchy and spacing changes can make a big usability difference."
        ]
      )
    );
  }
  ...
  if (project.page === "neurodivergent") {
    left.appendChild(
      createCaseStudySection("Problem", [
        "Many websites unintentionally increase cognitive load through cluttered layouts, dense text blocks, and inconsistent visual hierarchy.",
        "I wanted to design a cleaner experience that supports focus and reduces friction for neurodivergent users."
      ])
    );
    left.appendChild(
      createCaseStudySection("Research", [
        "I reviewed accessibility best practices and patterns from neurodiversity-centered UX guidance.",
        "The biggest needs were clearer structure, lower visual noise, and stronger readability."
      ])
    );
    left.appendChild(
      createCaseStudySection("Design Decisions", [
        "Used simplified page composition, stronger spacing systems, and predictable navigation patterns.",
        "Focused on typography and contrast choices that help scanning and comprehension."
      ])
    );
    left.appendChild(
      createCaseStudySection("Testing + Iteration", [
        "Refined component hierarchy and content grouping based on feedback from peers reviewing task flow.",
        "Adjusted layout rhythm and labeling to make the interface easier to understand on first pass."
      ])
    );
    left.appendChild(
      createCaseStudySection("My Role", [
        ...
        "Prototype construction in Figma"
      ])
    );
  }
...
function renderNeurodivergentProject(project) {
  const wrap = createEl("div", {}, []);
  const url = project.embed?.url || "";
  wrap.appendChild(createEl("div", { className: "mini-title", textContent: "Prototype + design artifact" }, []));
  wrap.appendChild(
    createEl("div", { className: "project-desc" }, [
      "This project is presented as a case-study style prototype artifact. Open the link to review wireframes and prototype direction."
    ])
  );
  const row = createEl("div", { className: "project-actions" }, []);
  const btn = createEl("button", { className: "btn btn-primary" }, ["Open project link"]);
